...
// var md5worker = toolbox.GetNewCryptoWorker(toolbox.CryptoMD5)
...
func LoggerWithRolling(logdir, filename string, maxdays int, skippath ...string) gin.HandlerFunc {
	...
	// 	if pathtool.IsExist(filepath.Join(f.logDir, fmt.Sprintf("%s.%v.%d.log", filename, t.Format(toolbox.FileTimeFormat), i))) {
	// 		f.fileIndex = i
	// 	}
	// }
	// // 创建新日志
	...
	// 	token := c.GetHeader("User-Token")
	// 	if len(token) == 36 {
	// 		token = md5worker.Hash(toolbox.Bytes(token))
	// 	}
	// 	c.Next()
	...
	// 			param.TimeStamp.Format(toolbox.ShortTimeFormat),
	// 			param.StatusCode,
	// 			param.Latency,
	// 			param.ClientIP,
	// 			param.Method,
...
// 	f.nameNow = fmt.Sprintf("%s.%v.%d.log", f.fname, t.Format(toolbox.FileTimeFormat), f.fileIndex)
// 	// 比对文件名，若不同则重新设置io
...
// 	}
// 	go func(s string) {
// 		err := toolbox.ZIPFile(f.logDir, s, true)
// 		if err != nil {
...
// 		if ex != nil {
// 			os.WriteFile("ginlogerr.log", toolbox.Bytes(fmt.Sprintf("clear log files error: %s", ex.Error())), 0664)
// 			return
// 		}
// 		t := time.Now()
...
// 	if f.err != nil {
// 		os.WriteFile("ginlogerr.log", toolbox.Bytes("Log file open error: "+f.err.Error()), 0664)
// 		f.out = io.MultiWriter(os.Stdout)
// 	} else {
// 		if gin.Mode() == "debug" {
