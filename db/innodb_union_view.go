...
func (d *Conn) AlterUnionView(dbname, tableName, alterStr string, maxSubTables int) error {
	...
	var ans *QueryData
	// 找到所有以指定命名开头的所有表
	strsql = "select table_name from information_schema.tables where table_schema=? and table_type='BASE TABLE' and table_name like '" + tableName + "%' order by create_time desc,table_name desc"
